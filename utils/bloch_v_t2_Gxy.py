...
class Gxyt2fBlochsim_fast_B0_jit(jit.ScriptModule): #online learing faster 2D simulation
    ...
    def forward(self, rRF, iRF, Gx, Gy, B_0, B_1):
        sRF_dt = self.dt
        sM_x = torch.linspace(-self.FOV/2, self.FOV/2, self.nvox).cuda() #[128]
        ...
        for i in range(rRF.size(-1)):
            m = torch.cat((mx.unsqueeze(-1), my.unsqueeze(-1), mz.unsqueeze(-1)), -1) #[B,nvox,nvoy,t2,3]
            m = m.reshape([m.size(0), m.size(1)*m.size(2)*m.size(-1),1])#[B,nvox*nvoy*3,1]
            ...
            R_ = bb.transpose(2, 3).permute(0, 2, 3, 1)  # [b,3,3,nvox*nvoy]
            R = torch.diag_embed(R_, 0, 1, 2)  # [b,nvox*nvoy,nvox*nvoy,3,3]

            R = R.reshape([rRF.size()[0], self.nvox*self.nvoy, self.nvox*self.nvoy * 3, 3]).permute(0, 1, 3, 2).reshape(
                [rRF.size()[0], self.nvox*self.nvoy * 3, self.nvox*self.nvoy * 3]).transpose(2, 1) #[b,nvox*nvoy * 3,nvox*nvoy * 3]

            #m = torch.matmul(R, m)#.cuda()#[B,3nvox*nvoy,3nvox*nvoy] X [B,3nvox,1]
            ...
            # if len(self.t2)>1:
            #     mx = mx * torch.exp(-self.dt / self.t2).cuda()
            #     my = my * torch.exp(-self.dt / self.t2).cuda()
    ...
    def forward(self, rRF, iRF, Gx, Gy):
        ...
        for i in range(rRF.size(-1)):
            ...
            b1 = torch.einsum('b,bs->bs', torch.sqrt(rRF[:,i]**2+iRF[:,i]**2),
                              torch.ones(rRF.size(0), self.nvox*self.nvoy).cuda()) #[b, 1, 256]
            beff = torch.sqrt(b1 ** 2 + sM_f ** 2)
